...
$.ajaxSetup({
	...
	/* BUTTONS TEMPLATES */
		...
		$(document).on( "click", ".btn-add-to-folder", function(){					
	    		var data = [{track:$(this).attr("data-track"), artist:$(this).attr("data-artist"), cover:$(this).attr("data-cover")}];	    			    		
...
{
	setInterval(function(){getActividySider()}, 20000);
}
...
function getVideo(track,artist,obj)
{
	...
	$("#trackInfo").text("");
	searching = true;	
	$.getJSON(base_url+'music/getYoutube/', {track:track,artist:artist,picture:window.btoa($(obj).attr("data-cover"))}, function(json, textStatus) {		
		if(json.id != 'null' && json.id != null)
		{
			...
			{
				$.post(base_url+'music/musik/get_subtitle',{v:json.id,track:track,artist:artist}, function(data, textStatus, xhr) {
					subtitle = data;
...
function register_user()
{
	...
	$.post(base_url+'music/registerUser', {"nick":nick,"email": email,"pwd1":pwd1, "pwd2":pwd2}, function(data, textStatus, xhr) {
		if(data.error == 1)
		{
			alert(data.msg);
